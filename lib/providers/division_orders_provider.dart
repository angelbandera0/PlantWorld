...
class DivisionOrdersProvider {
  //contiene todas las variables globales
  Global global = Global();
  String dataType = "division_orders";

  //retorna el genero segun el id
  Future<DivisionOrder> getDivisionOrder(int id) async {
    final url = Uri.https(
        global.url,
    ...
    DivisionOrder data =
        DivisionOrder.fromJsonMap(jsonDecode(response.body)['data']);
    return data;
  }

  //returna el listado de division orders
  Future<ListDivisionOrder> getListDivisionOrderByString() async {
    ...
    ListDivisionOrder list = ListDivisionOrder.fromJsonList(
    ...
    for (int i = 2; i <= lastPage; i++) {
      ...
      ListDivisionOrder aux =
          ListDivisionOrder.fromJsonList(jsonDecode(response.body));
